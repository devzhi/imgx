...
var (
	...
	os        = flag.String("os", "linux", "operating system of the image")
)
...
func main() {
	...
	// 获取镜像清单
	manifest, err := pull.GetManifest(token, *imageName, *tag, *arch, *os)
	if err != nil {
		fmt.Println("Error getting manifest", err)
		return
	}
	// 下载镜像
	path, err := pull.DownloadImage(token, manifest, *arch, *os, *imageName, *tag)
	if err != nil {
	...
	// 删除临时文件
	defer pull.RemoveImageSaveDir(*imageName, *tag, *arch, *os)
	// 打包镜像
	outputFile, err := pull.Package(*path, *imageName, *tag, *arch, *os, nil)
	if err != nil {
